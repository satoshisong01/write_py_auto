...
// === 단일 계정을 처리하는 함수 (키워드를 파라미터로 받음) ===
const processAccount = async (
  account,
  browser,
  accountKeywords, // === 변경: 서버에서 새로 가져오는 대신, 여기로 미리 받은 키워드 목록을 주입
  accountUrlList,
  currentApiKey
) => {
  ...
  // === 변경: 기존 while문으로 계속 키워드 요청하던 로직 -> for-of 루프로 변경
  for (const keyword of accountKeywords) {
    if (globalSuccessCount >= totalTargetCount) {
      break; // 전체 목표치 도달 시 중단
    }

    // 동시성 제한을 위해 limit 안에서 수행
    await limit(async () => {
      // 키워드 처리
      let page;
      try {
        ...
        let postData = await generateGeminiContent(keyword, currentApiKey);
        ...
        if (copiedUrl) {
          ...
          const urlFilePath2 = path.resolve(__dirname, "copied_urls2.txt");
          await fs.appendFile(
            urlFilePath2,
            `${dateTimeString} ${username}: ${copiedUrl}\n`,
            "utf-8"
          );
          console.log(
          ...
          try {
            const insertResponse = await axios.post(
              `${apiBaseUrl}/api/posts/insert`,
              {
                write_date: dateTimeString,
                username: username,
                link: copiedUrl,
                write_mark: "O", // 예: "글 작성 완료" 같은 식으로
                py_date: null, // 지금은 py_date는 없으니 null
                py_mark: null, // 나중에 python 스크립트 작업시 업데이트
                naver_mark: null, // 나중에 네이버 작업시 업데이트
                title: title, // Gemini API로 생성된 postData.title
                used_keyword: keyword, // 현재 사용한 키워드
              }
            );

            if (insertResponse.data.success) {
              console.log(
                `[DB] 게시물 정보가 DB에 저장되었습니다. (username=${username})`
              );
            } else {
              console.error(`[DB] 저장 실패: ${insertResponse.data.message}`);
            }
          } catch (error) {
            console.error("[DB] Insert API 호출 중 에러:", error.message);
          }
          ...
          // (1) 200개마다 프롬프트 교체
          if (globalSuccessCount % 200 === 0) {
            try {
              // 1) 랜덤 프롬프트 가져오기
              const randomPromptRes = await axios.get(
                `${apiBaseUrl}/api/prompt-status/random`
              );
              if (randomPromptRes.data.success) {
                const randomPrompt = randomPromptRes.data.prompt;
                console.log(
                  `\n[자동 교체] 200개 작성 달성 → 새로운 랜덤 프롬프트로 교체:`
                );
                console.log(randomPrompt);

                // 2) gemini.js 교체
                const updateRes = await axios.post(
                  `${apiBaseUrl}/api/scripts/updatePrompt`,
                  {
                    prompt: randomPrompt,
                  }
                );
                if (updateRes.data.success) {
                  console.log("[자동 교체] 프롬프트 업데이트 성공!");
                } else {
                  console.error(
                    "[자동 교체] 프롬프트 업데이트 실패:",
                ...
                console.error(
                  "[자동 교체] 랜덤 프롬프트 API 실패:",
                  randomPromptRes.data.message
                );
              }
            } catch (err) {
              console.error("[자동 교체] API 호출 중 오류:", err.message);
            }
          }
...
(async () => {
  try {
    ...
    // === 변경: 한 번에 전체 키워드 가져오기 ===
    console.log("키워드 전체 목록 가져오는 중...");
    const keywordResponse = await axios.get(
      `${apiBaseUrl}/api/keywords/keywordEnd`
    );
    let globalRemainingKeywords = keywordResponse.data.data.map(
      (k) => k.keyword
    );

    if (globalRemainingKeywords.length === 0) {
    ...
    const accountTasks = accountsWithQuota.map((account) =>
      accountLimit(async () => {
        const { username, keywordLimitPerAccount } = account;

        // === 변경: 전역 키워드 배열에서 이 계정이 사용할 키워드 추출 ===
        const accountKeywords = globalRemainingKeywords.slice(
          0,
          keywordLimitPerAccount
        );
        // 전역 배열에서 그만큼 제거
        globalRemainingKeywords = globalRemainingKeywords.slice(
          keywordLimitPerAccount
        );

        // 만약 뽑은 키워드가 없으면 종료
        if (accountKeywords.length === 0) return;
